...
function Event({ name, startTime, endTime, priority, id, day, duration, boardRef }) {
  ...
  const prevStateRef = useRef({eventState: eventState})

  // console.log('Board offset:', boardRef.current.offsetWidth);
  // console.log('Board offset height:', boardRef.current.offsetHeight);
  // console.log((dayjs(startTime, "h:mm A")));
  ...
  useEffect(() => {
    prevStateRef.current.eventState = eventState;
  }, [eventState])
  ...
  useEffect(() => {
    ...
    let moveTimeStart = lastResize.position.y / 48.5
    if (lastResize.direction === 'top') {
    ...
    if (lastResize.direction === 'bottom') {
      moveTimeStart = dayjs(startTime, "hh:mm A")
      moveTimeStart.year(refDay.get('year')).month(refDay.get('month')).date(refDay.get('date'));
      let moveEndMinutes = lastResize.delta.height / 48.5 * 60
  ...
  useEffect(() => {
    const prevEventState = prevStateRef.current.eventState

    console.log('This is newState:', eventState)
    const refDay = dayjs(day, "MM/DD/YYYY");
    ...
    //do nonsense to calc endTime
    let moveTimeEndDelta = (eventState.y - prevEventState.y) / 48.5
  ...
  return (
    <div>
      <Rnd
        ...
        onDragStop={(e, d) => {
          console.log('This is the drag coord:', d)
          const previousHeight = eventState.y;
          setState((prevState) => {
            const newState = {
              width: '100vw',
              height: duration * 8.5,
